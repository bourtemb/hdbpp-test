...
import static org.tango.hdb_configurator.diagnostics.StatisticsDialog.firstColumnBackground;
...
public class ServerInfoTable extends JDialog {
	...
	private static final String[] columnNames = { "Server", "Host", "Uptime"};
	private static final int[] columnWidth = { 300, 80, 200 };
	private static final int SERVER_NAME   = 0;
	private static final int SERVER_HOST   = 1;
	private static final int SERVER_UPTIME = 2;
	private static final Font tableFont = new Font("Dialog", Font.BOLD, 12);
	 ...
	 */
	//===============================================================
	public ServerInfoTable(JFrame parent,
                           SubscriberMap subscriberMap, DeviceProxy configuratorProxy) {
		super(parent, true);
		this.parent = parent;
		initComponents();
		...
		serverInfoList.add(new ServerInfo("Configurator", configuratorProxy, true));
	...
	private void computeColumnWidth() {
		...
		for (ServerInfo serverInfo : serverInfoList) {
			if (serverInfo.name.length()>str.length())
				str = serverInfo.name;
		}
		//	Display in a label
		JLabel lbl = new JLabel(str);
		lbl.setFont(tableFont);
		getContentPane().add(lbl, BorderLayout.EAST);
		pack();
		//	get the label width
		columnWidth[SERVER_NAME] = lbl.getWidth()+20;

		//	check for uptime
		str = "";
		for (ServerInfo serverInfo : serverInfoList) {
			if (serverInfo.name.length()>str.length())
				str = serverInfo.uptime;
		}
		lbl.setText(str);
		pack();
		columnWidth[SERVER_UPTIME] = lbl.getWidth()+20;
	...
	*/
	...
	public static void main(String args[]) {
		try {
			...
			new ServerInfoTable(null, subscriberMap, configuratorProxy).setVisible(true);
		}
		catch(DevFailed e) {
